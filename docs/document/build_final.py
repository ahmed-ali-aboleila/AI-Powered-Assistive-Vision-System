...
output_path = r'D:\Final Project\Logic_System\emotion & face recognition final_v6\final_report_v6_Vision_FINAL.docx'

doc = Document(input_path)

# Helper: add paragraph without style issues
...
# Step 3: Add Abstract, Keywords, Related Work at the beginning
...
# Step 4: Add inline references after relevant sections
for para in doc.paragraphs:
...
# Step 5: Rebuild References Section
in_refs = False
refs_to_remove = []
for para in doc.paragraphs:
...
for para in doc.paragraphs:
    if para.text.strip() == "10. References":
        ...
        for ref in new_refs:
            p = insert_after(para, ref)
        break

# Step 6: Add appendix sections at the end
last_para = doc.paragraphs[-1]

insert_after(last_para, "References by Section", bold=True, size=16)
insert_after(last_para, "The following references are grouped by the section they support, for clarity and ease of verification.")
...
insert_after(last_para, """The Assistive Vision System handles sensitive biometric data. The following ethical principles govern the design:

1. Privacy and Consent: All face data is stored locally. No images or embeddings are transmitted to external servers. Users must provide explicit consent before registering any person.

2. Data Security: The face database is protected by file-system permissions. No cloud storage is used for face recognition inference.

3. Bias and Fairness: The model was trained on diverse datasets, but performance may vary across ethnicities, ages, and lighting conditions.

4. Non-Discrimination: The "block" feature is for privacy preferences, not discrimination. System logs are stored locally.

5. Accessibility: The system is designed for visually impaired users with full audio feedback and voice control.""")
...
print("Saved final report:", output_path)
